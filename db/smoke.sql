...
INSERT INTO products (seller_id, name, description, price, stock_qty, status)
SELECT s.id, p.name, p.description, p.price, p.stock_qty, p.status
FROM (VALUES
  ...
  ('seller_aurora@shop.ua',
   ...
   199.00, 0, 'archived')
) AS p(seller_email, name, description, price, stock_qty, status)
JOIN users s ON s.email = p.seller_email;

INSERT INTO orders (buyer_id, status, currency, total_amount, created_at)
SELECT u.id, o.status, 'UAH', o.total_amount, o.created_at
FROM (VALUES
  ('BuyerOne@Example.COM', 'pending', 3499.00, now() - interval '2 days'),
  ('BuyerOne@Example.COM', 'placed', 2799.00, now() - interval '10 days'),
  ('BuyerOne@Example.COM', 'cancelled', 199.00, now() - interval '40 days')
) AS o(buyer_email, status, total_amount, created_at)
JOIN users u ON u.email = o.buyer_email;

INSERT INTO order_items (order_id, product_id, product_name, quantity, unit_price, line_total)
SELECT o.id, p.id, p.name, 1, p.price, p.price
FROM orders o
